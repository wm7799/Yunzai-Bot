...
export default class loginEvent extends EventListener {
  ...
  /** 扫码登录现在仅能在同一ip下进行 */
  async qrcode (event) {
    logger.mark(`请使用登录当前QQ的手机${logger.green('扫码')}完成登录，如果显示二维码过期，可以按${logger.green('回车键（Enter）')}刷新，重新输入密码请执行命令：${logger.green('npm run login')}`)
    // logger.info('等待扫码中...')
  ...
  async slider (event) {
    ...
    let txhelper = {
      url: event.url.replace('ssl.captcha.qq.com','txhelper.glitch.me')
    }
    ...
    if (ret.type == '2.滑动验证app请求码') {
      txhelper.req = await fetch(txhelper.url).catch((err) => logger.error(err))
      if (txhelper.req){
      ...
      txhelper.res = await fetch(txhelper.url).catch((err) => logger.error(err))
